...
### 1. Intended Purpose
**System:** Nova Sentinel — Autonomous Security Intelligence Platform  
...
- **Contact:** See Nova Sentinel documentation for governance and escalation paths.

---
*EU AI Act Conformity Addendum — Generated by Nova Sentinel*
`;
  ...
  const handleDownloadEUAIActPack = () => {
    ...
    const link = document.createElement('a');
    link.href = url;
    link.download = `nova-sentinel-eu-ai-act-pack-${incidentId || 'analysis'}-${new Date().toISOString().split('T')[0]}.md`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
